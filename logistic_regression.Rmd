...
# LD is more efficient (and unbiased) relative to multiple imputation.
...
```{r}
# In real world, this is what we are given. And then we have to fit logistic 
# regression model [using glm function] 
# to this following imputation [since we are doing a simulation,
# we know the true regression model values.]
data <- create.data(n = 1000)
data
```
...
```{r}
mod <- with(data_mis, glm(Y ~ x1 + x2, family = "binomial"))
```
