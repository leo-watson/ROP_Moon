...
* Mathematically: $P(R_i|Y_i)=P(R_i) = \phi$, where $Y_i$ is the i-th data point in our dataset $Y$, $R_i$ is an indicator for missingness ($R_i$ = 0 if not missing, $R_i$ = 1 if missing), $\phi$ is some parameter of the missing data model.  
...
* Yields unbiased estimators for regression weights under MAR, if the variable causing the MAR is part of the regression model. (e.g. if Solar.R was responsible for missingness in Ozone in the abobe example)
