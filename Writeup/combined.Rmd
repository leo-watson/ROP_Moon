---
title: "Missing Data Mechanisms & Imputation Methods"
output:
...
* A **MCAR** missing data mechanism is observed when missingness is some constant $\phi$ for all observations in the sample (@van2018flexible).  

* Mathematically: $P(R_i|Y_i)=P(R_i) = \phi$ (@van2018flexible). Here, $Y_i$ is the response for the $i^{th}$ individual in our sample, $R_i$ is an indicator for missingness ($R_i$ = 0 if $Y_i$ is not missing, $R_i$ = 1 if $Y_i$ is missing), $\phi$ is some parameter of the missing data model. 
  + Note that covariates $(X_1, ..., X_n)$ may also be under a MCAR mechanism with anaolgous notation.
...
* A **MAR** mechanism is observed when missingness is dependent on some observed variable of the data. In this case, the probability of missingness is dependent on groups defined by the data (@ritvikmath2018).  

* Mathematically: $P(R_i|Y_i) = P(R_i|Y_{i,o})$. (@statswithmia2020) I.e probability of missingness for the $Y_i$ observation is dependent on some characteristic of the observed data for that observation.   
...
* Most students joined a class from day 1, but some students joined late from the waitlist due to capacity restrictions. 10% of students who joined on time had a missing submission for the first problem set, while 30% of students who joined late missed the first problem set.
  + Mathematically, can think of each student as an observation. Let whether they joined from the waitlist or not be binary covariate $X_1$, and first problem set submission be binary response $Y$. $X_1$ determines the probability that a student's first problem set is missing, hence this is a MAR mechanism example. 



## MNAR
* A **MNAR** mechanism is observed when  missingness is dependent on the true value of the data point which we don't know for all subjects.

* Mathematically: $P(R_i|Y_i) \neq P(R_i|Y_{i,o})$ (@statswithmia2020). I.e. probability of missingness doesn’t depend on observed data; it depends on unobserved data which we don't know!
...
### Description
* Eliminates all observations containing ANY missing values in variables of interest (@van2018flexible).
...
* Unbiased estimates of means, variances, and regression coefficients under MCAR missingness (@van2018flexible).
* Can be efficient and exceedingly effective in specific scenarios:
    + (1): If probability to be missing does not depend on response $Y$; missing data rate may depend on any predictors $X_i$, with missing data in both Y/X allowed (@van2018flexible).
    + (2): If complete data model is logistic regression. If missing data is confined to one of $Y$ or $X_i$, regression coefficients are unbiased if missingness depends only on $Y$ and not on $X_i$ (@van2018flexible).
...
* If data isn’t MCAR, generates severely biased estimates of means, variances, and regression coefficients (@van2018flexible). 
...
* First, pairwise deletion calculates means of variables using observed data for each variable (@van2018flexible).
* Then, combines this with correlations/covariances between the different variables in the dataset to impute values (@van2018flexible).
...
### Pros
* Pairwise deletion works well if data $(Y, X_1,...,X_n)$ approximately follows a multivariate normal distribution and the correlations between variables are low (@van2018flexible).
* If MCAR is plausible, provides unbiased estimates effectively and efficiently (@van2018flexible).


### Cons  
* Requires MCAR assumption for unbiased estimates.
* Correlation matrix obtained via pairwise deletion may give mathematically impossible results, and hence unable to perform further regression analysis (@mikko2020). [see code below]
...
* Moreover, the covariance/correlation matrix may not be positive definite; this is a requirement for common multivariable procedures and further analysis, e.g. MANOVA: multivariate analysis of variance (@gregory1998).
* The sample size will almost certainly vary between variables due to missingness. This is problematic for calculating standard error $= \frac{\sigma}{n}$ as the $n$ value is ambiguous and hence standard error will almost certainly be underestimated/overestimated (@mikko2020).


# Mean Imputation

### Description
* For each variable in the dataset, replaces missing values with mean of the observed values (@van2018flexible).
...
* Underestimates the variance due to imputed values having zero contribution to variance (@van2018flexible).
* Biases almost all estimators irrespective of missing data mechanism (@van2018flexible).
* Even biases the mean estimator $\bar{X}$ if not MCAR (@van2018flexible).
...
### Description
* First, fits a regression model between observed values of predictor variables $(X_1, ..., X_n)$ and response variable $Y$.
* Then, imputes NA values with the exact predicted values from regression model (@van2018flexible).
  + Note that this method can be be used to impute missing values in predictor(s) $X_i$ by fitting a regression model between observed $(X_1, \dots , X_{i-1}, X_{i+1}, \dots , X_n, Y)$ and $X_i$. 
...
### Pros
* Yields unbiased mean estimates under MCAR, and regression weights of imputation model if the explanatory variables are complete (@van2018flexible).
* Yields unbiased estimators for regression weights under MAR, if the variable causing the MAR is part of the regression model (@van2018flexible); (e.g. if Solar.R was responsible for missingness in Ozone in the above example)


### Cons
* Since imputed values are the exact predicted values from the regression model, correlation is overestimated and variability is underestimated (@joachim2021).
* Imputations are realistic if predicted linear model is almost perfect, but deterministic regression is often too good to be true as the uncertainty in the imputed values is not accounted for (@van2018flexible).

# Stochastic Regression Imputation

### Description
* Very similar to deterministic regression imputation but as an extra step adds noise (random draw from residual) to predictions to account for the uncertainty in imputation (@van2018flexible).
...
*Recall the deterministic regression imputation plot (below for conveninence)* 
*and note the random noise added to imputed values above:*   

![](images/deterministic_regression_imp.png)

### Pros
* Unlike deterministic regression imputation, preserves correlation between variables (@joachim2021).
...
* This model assumes equal dispersion of data for the entire linear model. Hence, not accurate for heteroscedastic (non-constant variance) distributions (@van2018flexible). In the example above, observed dispersion is more extreme in the 200~300Ly Solar.R band, but imputed values do not acocunt for this and follow same degree of dispersion throughout.
...
* It imputes missing values by replacing them with the latest observed value (@van2018flexible). Typically seen in clinical trials
...
### Cons
* Should only be applied when we are confident in cases where we are certain what the missing values should be (@liu2015methods, @national2010prevention).
* Can yield biased estimators even under MCAR (@liu2015methods).
...
* As per its name suggests, BOCF imputes missing values by replacing them with some predetermined baseline value; typically used in clinical trials where a "baseline value" can easily be established (@deng2021).
...
### Cons
* Should only be applied when we are confident in cases where we are certain what the missing values should be (@deng2021).
* Can yield biased estimators even under MCAR (@national2010prevention).
* The US National Reserach Council Panel on Handling Missing Data in Clinical Trials recommended in 2010 that BOCF shouldn't be used unless under very specific circumstances where BOCF assumptions are justified.
...
* Imputation technique applicable when explanatory variable(s) have missingness but response does not (@van2018flexible).
* Missing values are replaced by a fixed value (usually 0 or mean imputation), and a missingness indicator variable is added to each observation in the dataset for each incomplete variable (@van2018flexible).
...
### Pros
* In randomized trials, can be used to generate unbiased estimators in MCAR and MAR covariate situations (@groenwold2012missing).
* Retains the whole observed dataset.

### Cons
* In nonrandomized studies (e.g. observational studies), this method can result in biased estimators and associations between variables even in MCAR situations (@groenwold2012missing).
...
1. Imputation method for multivariate data that takes an incomplete dataset as input and creates multiple copies of the complete data (@van2018flexible).
  
2. Then, impute incomplete columns with plausible values given other columns through an iterative predictive method (methods include predictive mean matching, random forests, mean imputation, etc.) (@sam2021).
    + Iterative methods are used until imputed values converge (typically 5-10 iterations is sufficient) (@rachit2020).  
  
3. Next, obtain an estimate for the parameter of interest for each version of the imputed dataset (@van2018flexible).
    + This is done using regular analysis techniques similar to the other imputation procedures.
  
4. Finally, pool estimators together to create a single pooled estimate (@van2018flexible).
...
* Number of imputations should be equal to %-age of missingness as a rule of thumb (@van2018flexible).
...
* The *mice()* function applies multiple imputation. The *m* parameter refers to the # of datasets we want to create, i.e. the # of circles in the 2nd and 3rd steps of the schematic above (@mice2011).
```{r}
# [step 1 above]
...
### Pros:
* Gives unbiased and confidence-valid estimators under MAR, MCAR (@van2018flexible).  

* Highly versatile and general technique. Can accommodate situations with low confidence in missing values by having a large number of imputed dataset copies *m*; similarly, can accomodate situations with high confidence in missing values by having a small number of imputed dataset copies *m* (@rachit2020).  

* Produces a suitable standard error value (listwise deletion produces too large standard error, other techniques produce too small standard error) close to true value (@van2018flexible).

### Cons:

* Ad-hoc methods specified above may work better & faster in edge cases (@groenwold2012missing). For example, listwise deletion is equivalent and faster than multiple imputation if missing values occur only in the outcome.
  + This is because if missingness is only in the outcome, then missing data model is the same as the prediction model as no predictors are imputed (@van2018flexible). Hence, multiple imputation will simply perform complete-case analysis in a roundabout way.
